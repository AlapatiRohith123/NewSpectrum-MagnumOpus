...
function ProblemsHome(){
    ...
    {
        let filters=document.getElementsByClassName("filtersList");
        ...
        let localdata=fulldata;
        if(a.length!=0)
        {
            for(let i in localdata)
            {
                if(!a.includes(localdata[i].Field))
                {
    ...
    async function fetchData()
    {
        ...
        for(let i of data)
        {
            ...
            id.innerHTML=i._id;
            boxClone.querySelector(".fieldName").innerHTML=i.Field;
